...
static void CAN_SendHeartbeat(void) {
	...
	HAL_StatusTypeDef st = HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &canTxHeader,
			payload);
	...
	if (st == HAL_OK) {
		...
		if (!canFirstFailPrinted) {
			...
			CDC_Print(
					"  CAN PSR: LEC=%lu DLEC=%lu ACT=%lu EP=%lu EW=%lu BO=%lu\r\n",
					...
					(unsigned long) psr.Warning, (unsigned long) psr.BusOff);
		}
	}

	CDC_Print("CAN TX #%u: %s  (pass=%lu fail=%lu)\r\n", (unsigned) canTxSeq,
			HalStr(st), (unsigned long) canTxPass, (unsigned long) canTxFail);
...
static void PrintVal(char *dest, int maxLen, float v) {
	int vi = (int) (v * 1000);
	int va = vi < 0 ? -vi : vi;
	snprintf(dest, maxLen, "%c%d.%03d", v < 0 ? '-' : '+', va / 1000,
			va % 1000);
}
...
int main(void) {
	...
	while (1) {
			...
			if (chan == CH_INPUT) {
				...
				while (idx < cargoLen) {
					...
					if (id == RPT_BASE_TS || id == RPT_TS_REBASE) {
						...
						lastX = (float) (int16_t) ((uint16_t) c[idx + 4]
								| (uint16_t) c[idx + 5] << 8) / 512.0f;
						lastY = (float) (int16_t) ((uint16_t) c[idx + 6]
								| (uint16_t) c[idx + 7] << 8) / 512.0f;
						lastZ = (float) (int16_t) ((uint16_t) c[idx + 8]
								| (uint16_t) c[idx + 9] << 8) / 512.0f;
						gyroCount++;
						idx += 10;
					} else {
		...
		if (now - lastPrint >= 1000) {
			...
			CDC_Print(
					"X:%s  Y:%s  Z:%s  |  ok=%lu fail=%lu gyro=%lu  I2C=%s\r\n",
					sx, sy, sz, (unsigned long) readOK,
					(unsigned long) readFail, (unsigned long) gyroCount,
